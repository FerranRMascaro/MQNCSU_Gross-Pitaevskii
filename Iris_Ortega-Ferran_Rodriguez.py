...
if __name__ == '__main__':
    ...
    ctf = 1 #constant multiplyin the kinetic energy term (to take into accoun the Thomas-Fermi approximation)
    # changes for the armonic oscillator case
    if arguments.armonic_osci:
        log_path = log_path + 'ao_'
        cequ = 0
    # changes for the Thomas-Fermi approximation
    ...
    for num_run in range(len(indata['N'])):
        # initiating the vectors with 0 value
        ...
        # N_steps=n1, N=aa
        # extracting the data for this run from the pandas dataframe
        ...
        # Logs where we will write the output for mu and the density
    ...
    # ************************************
        ...
        for it in range(int(iter)):
            ...
            for i in range(N_steps):
                freo[i] = fren[i]/xnorm
        # saving the computed values of the chemical potential for this run 
        ...
        # saving the computed values of the density for this run 
        for i in range(1, N_steps):
            log_den.write(str(xr[i]) + '\t' + str(den[i]) + '\n')
        log_den.close()

        # saving the computed values in a new dataframe for our results
        pd_res.loc[num_run] = [N, xnormden, ene0, chem, xkin, pot, poth0, potself, radious, radious2]

    #  dumping the results from the dataframe to a
